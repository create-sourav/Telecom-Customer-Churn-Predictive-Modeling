...
class_names = {0: "Churned", 1: "Joined", 2: "Stayed"}
results = []
...
for name, model in models.items():
    ...
    # Per-class metrics
    precision = precision_score(y_test, y_test_predictions, average=None, zero_division=0)
    recall    = recall_score(y_test, y_test_predictions, average=None, zero_division=0)
    f1        = f1_score(y_test, y_test_predictions, average=None, zero_division=0)
    roc_auc   = roc_auc_score(y_test, y_test_probabilities, multi_class="ovr", average=None)
...
# Predictions
y_train_pred = GradientBoosting_tuned.predict(x_train_resample)
y_train_prob = GradientBoosting_tuned.predict_proba(x_train_resample)

y_test_pred = GradientBoosting_tuned.predict(x_test_scaled)
y_test_prob = GradientBoosting_tuned.predict_proba(x_test_scaled)
...
roc_auc   = roc_auc_score(y_test, y_test_prob, multi_class="ovr", average=None)
...
predicted_labels = label.inverse_transform(predicted_class)

# Get churn probability column
churn_index = list(label.classes_).index("Churned")
churn_probability = predicted_probabilities[:, churn_index]

# Build final prediction dataframe
df_predictions = pd.DataFrame({
    "Customer ID": customer_ids,
    "Actual Status": label.inverse_transform(y),
    "Predicted Status": predicted_labels,
    "Churn Probability": churn_probability
})

# Adjusted prediction using optimal threshold
...
predicted_class_labels = label.inverse_transform(predicted_class_numbers)
