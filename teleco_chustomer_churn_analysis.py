...
# 2. Create Geo-Clusters from Latitude & Longitude
...
# 3. Define features X and target y. Do NOT modify df itself.
...
# Get the index for 'Churned' class
churned_class_index = list(encoders["Customer Status"].classes_).index("Churned")

# Compute ROC for the 'Churned' class
fpr, tpr, thresholds = roc_curve(
    y_test,
    RandomForest_tuned.predict_proba(x_test_scaled)[:, churned_class_index],
    pos_label=churned_class_index
)
...
print(f"Optimal Churn Probability Threshold (Youden's J): {optimal_threshold:.3f}")
...
# Final Full Dataset Prediction

# Use the tuned model for full prediction
model = RandomForest_tuned
X_scaled_full = scaler.transform(X) # Scale full data

# Churn probability for each customer (index 'churned_class_index')
churn_prob_full = model.predict_proba(X_scaled_full)[:, churned_class_index]
y_pred_full = model.predict(X_scaled_full) # Default prediction

# Create final prediction DataFrame
full_predictions_df = pd.DataFrame({
    "Customer ID": customer_ids,
    "Actual_Status": encoders["Customer Status"].inverse_transform(y),
    "Predicted_Status": encoders["Customer Status"].inverse_transform(y_pred_full),
    "Churn_Probability": churn_prob_full
})

# Apply the optimal threshold to create the adjusted prediction column
full_predictions_df["Predicted_Status_Adjusted"] = np.where(
    full_predictions_df["Churn_Probability"] >= optimal_threshold,
    "Churned",
    full_predictions_df["Predicted_Status"]
)

print("\n--- Prediction Review (Top 10) ---")
display(full_predictions_df[[
    "Customer ID",
    "Actual_Status",
    "Churn_Probability",
    "Predicted_Status",
    "Predicted_Status_Adjusted"
]].head(10))

# Sort customers by churn probability (descending)
results_sorted = full_predictions_df.sort_values(
...
df["Probability"] = churn_prob_full
