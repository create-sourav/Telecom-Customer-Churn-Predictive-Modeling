...
"""## Outlier handling before machine learning model run"""
...
"""After fine-tuning, training log loss dropped from ~0.41 to 0.21, while test log loss reduced more modestly from ~0.48 to 0.38, increasing the train–test gap. This indicates higher model capacity and controlled variance, not an error, and is expected as the model becomes more expressive.
...
If train loss is very low but test loss is much higher (≥2×), the model is overfitting and learning noise.

Accuracy gap should be interpreted with test accuracy: a higher gap is acceptable if test accuracy improves.

Acceptable accuracy gap is ~3–8% (good) and ~8–15% (controlled overfitting); beyond 15–20% indicates severe overfitting.
...
Stop tuning when test metrics stop improving, not when the gap becomes zero—zero gap usually indicates underfitting.
...
### 🧭 Interpretation
- Customers with Churn Probability ≥ 0.3 are high-risk attriters.
